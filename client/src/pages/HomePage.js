...
function HomePage() {
  ...
  const handleGenerateSupply = () => {
      ...
      let initialSupply = allCards.filter(card => requiredCardNames.includes(card.name_jp));
      let tagsToFulfill = activeTags.filter(tag => !initialSupply.some(card => card.tags.includes(tag)));
      if (requiredCardNames.length + tagsToFulfill.length > 10) { alert('必須カードと、必須カードでは満たされないタグの合計が10を超えています。条件を減らしてください。'); return; }
      let supply = [...initialSupply];
      let candidatePool = allCards.filter(card => !bannedCards.includes(card.name_jp)).filter(card => card.cost >= costRange.min && card.cost <= costRange.max);
      for (const tag of tagsToFulfill) {
          let roleCandidates = candidatePool.filter(candidate => candidate.tags.includes(tag) && !supply.some(sc => sc.name_jp === candidate.name_jp));
          if (roleCandidates.length === 0) { continue; }
          const randomIndex = Math.floor(Math.random() * roleCandidates.length);
          const pickedCard = roleCandidates[randomIndex];
          supply.push(pickedCard);
      }
      let remainingPool = candidatePool.filter(candidate => !supply.some(supplyCard => supplyCard.name_jp === candidate.name_jp));
      while (supply.length < 10 && remainingPool.length > 0) {
          let weightedPool = remainingPool.map(card => {
              let weight = 1.0;
          ...
          if (nextCard) {
              ...
              remainingPool = remainingPool.filter(c => c.name_jp !== nextCard.name_jp);
          } else {
              const randomIndex = Math.floor(Math.random() * remainingPool.length);
              const randomCard = remainingPool.splice(randomIndex, 1)[0];
          ...
          <div className="filter-container">
              ...
              <div className="filter-section">
                  ...
                  <div className="card-status-list">
                      {allCards.map(card => (<div key={card.name_en} className={`card-status-item ${cardStatuses[card.name_jp]}`} onClick={() => handleCardStatusChange(card.name_jp)}>{card.name_jp}</div>))}
                  </div>
              </div>
              <div className="generate-button-wrapper">{renderButton()}</div>
          </div>
          <main className="supply-display">
              {appState === 'loading' && <p className="loading-message">バックエンドからカードデータを読み込んでいます...</p>}
              {appState === 'error' && (<div className="error-message"><p>データの読み込みに失敗しました。</p><p>バックエンドサーバーが起動しているか、ターミナルを確認してください。</p></div>)}
              {appState === 'success' && selectedSupply.length === 0 && (<p style={{gridColumn: '1 / -1'}}>フィルター条件を設定して、サプライを生成してください。</p>)}
                  ...
                  {submissionStatus === 'success' ? (
                      ...
                      <>
                          ...
                          <div className="star-rating" onMouseLeave={() => setHoverRating(0)}>
                              {[1, 2, 3, 4, 5].map(star => (<span key={star} className={star <= (hoverRating || rating) ? 'star active' : 'star'} onClick={() => setRating(star)} onMouseEnter={() => setHoverRating(star)}>★</span>))}
                          </div>
...
// 最終的にエクスポートするのは HomePageFinal ではなく HomePage
